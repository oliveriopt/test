...
def prepare_query_tasks(**kwargs):
    ...
    for table in tables:
        ...
        s_sql = f"""
            ...
            WHERE database_name = '{catalog}' AND schema_name = '{schema}' AND table_name = LOWER('{name}')
            LIMIT 1
        """
...
def build_flex_template_task(i, conf):
...
with DAG(
    dag_id="qa_flex_template_multiquery_parallel",
    ...
    prepare_query_tasks_op = PythonOperator(
        task_id="prepare_query_tasks",
        python_callable=prepare_query_tasks,
        provide_context=True,
    )

    start >> read_config >> prepare_query_tasks_op
    ...
    def create_flex_tasks():
        ...
        ti = context["ti"]
        task_inputs = ti.xcom_pull(task_ids="prepare_query_tasks", key="task_inputs")
        tasks = []
        for i, conf in enumerate(task_inputs):
            task = build_flex_template_task(i, conf)
            prepare_query_tasks_op >> task
            tasks.append(task)
        return tasks

    create_tasks = PythonOperator(
        task_id="create_flex_tasks",
        python_callable=create_flex_tasks,
        provide_context=True
    )

    prepare_query_tasks_op >> create_tasks
